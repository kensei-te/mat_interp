...
wdirin = st.sidebar.text_input(label="if not specified, filename is used for foldername",value=None)
st.sidebar.write(" ")
st.sidebar.write('step0b: upload teacher data csv:')
inputf = st.sidebar.file_uploader(label='csv!', type=['csv'])
if inputf is None:
    inputf = st.sidebar.text_input(label = "Or specify name of teacher data csv.\nWrite 'train_df.csv' to use existing work", value="")
...
with st.expander('click to expand data_preparation'):
    if inputf is not None:
        ...
        if st.checkbox('rawdata_plot'):
            fig = px.line(data_frame=df, x=X1_name, y=Y_name, color=X2_name,
        ...
        if st.checkbox('X2_selecion'):
            ...
            selected_2nd_axis = st.multiselect('select 2nd-axis-values for teacher',
                X2list, default=X2list)

            st.write(str(selected_2nd_axis))

            train_df = df[df[X2_name].isin(selected_2nd_axis)]
            fig = px.line(data_frame=train_df, x=X1_name, y=Y_name, color=X2_name,
            color_discrete_sequence=colormap)
            st.plotly_chart(fig)
        else:
        ...
        if st.checkbox("image plot of traindata"):
            ...
            traindata = go.Heatmap(z= df_train[Y_name].T,
                x=df_train[X1_name],
                y=df_train[X2_name])
...
with st.expander('click to expand model_construction'):
    ...
    if inputf is not None:
        ...
        n_trial = st.number_input(label="hyperparam optimization total trial number", value=n_trial_ini, min_value=1)
        n_cores = st.number_input(label="# of cores used parallel, make sure not to exceed total PC cores", value=n_cores_ini, min_value=1)
        ...
        if solve_option == "Stochastic Gradient Descent":
            NNsetting=dict(X1_name = X1_name,
                            X2_name = X2_name,
                            Y_name = Y_name,
                            n_trial = n_trial,
                            n_cores = n_cores,
...
with st.expander('click to expand check_learning_process'):
    ...
    if 'rangel' not in st.session_state:
        ...
        range_l = st.number_input(
            label = "lower range of graph for r2_score graph below",key="range_l",
    ...
    if check_hist:
        ...
        dfcheckrec = dfcheckrec.reindex(columns=["state", "value", "datetime_start", "datetime_complete", "duration"])
        check_fin = (dfcheckrec['state'] == "RUNNING").sum() # number of "running"
        fin_trial_n = (dfcheckrec['state'] == "COMPLETE").sum() + (dfcheckrec['state'] == "PRUNED").sum()
        if check_fin < 1:
        ...
        if checkrec_n>0:
            ...
            dftemp2 = pd.DataFrame(columns=['epochs','loss','trial_n','lr_n','batch_n'])
            for i in range(trial_num+1):
                ...
                ylist = list(yval)
                trial_n = [i]*len(ylist)
                lr_n = [float(str(study.trials[i]).split("'lr_ini': ")[1].split(",")[0])]*len(ylist)
                ...
                dftemp = pd.DataFrame(list(zip(xlist,ylist,trial_n,lr_n,batch_n,runstate)), columns=['epochs','r2_score','trial_n','lr_n','batch_n','runstate'])
                
                dftemp2 = pd.concat([dftemp2,dftemp], axis=0)

            fig_study = px.line(dftemp2, x='epochs',y='r2_score',color='trial_n',hover_data=['lr_n','batch_n','runstate'],log_y=False)
            ...
            st.write("best r2_score",y_min, "on trial:",best_trial_n, ", lr :",best_trial_lr, ", batch:", best_trial_batch)

            fig_study.update_layout(yaxis_range=[lny_min, lny_max], width=650, height=500)
    ...
    if check_result:
        ...
        st.write("minimum loss of ",study.best_value, "achieved on the following conditions")
        ...
        if st.checkbox("visualize learned curves"):
            ...
            colormap = sns.color_palette("gist_rainbow", N_COLORS).as_hex()
            fig = px.line(data_frame=train_df, x=NNsetting["X1_name"], y=NNsetting["Y_name"], color=NNsetting["X2_name"],
            color_discrete_sequence=colormap)
            fig.add_scatter(x=df_fitted[NNsetting["X1_name"]], y=df_fitted[NNsetting["Y_name"]], mode="markers", marker=dict(color="black", symbol="circle-open"), name="learned")
            st.plotly_chart(fig)
        ...
        if st.checkbox("image plot"):
            path_fitted = os.path.join(wdir,"df_fitted.csv")
            df_fitted = pd.read_csv(path_fitted)
            fitdata = go.Heatmap(z= df_fitted[NNsetting["Y_name"]].T,
...
with st.expander('click to expand use_model'):
    if inputf is not None:
        ...
        if os.path.exists(path_train) and os.path.exists(path_setting):
            ...
            if X1_step < 0.1:
                X1_step = 0.2
            X2_step = abs(float(df[NNsetting["X2_name"]].unique()[1]-df[NNsetting["X2_name"]].unique()[0]))
            if X2_step < 0.1:
                X2_step = 0.1
            ...
            n1 = st.number_input(
                label = "predict range: X1 minimum",key="n1",
                value = round(X1_min,1)
            )
            X1pred_min = n1

            n2 = st.number_input(
                label = "predict range: X1 maximum",key="n2",
                value = round(X1_max,1)
            )
            X1pred_max = n2

            n3 = st.number_input(
                label = "predict range: X1 step",key="n3",
                value = round(X1_step,1)
            )
            X1pred_step = n3

            st.write("learned range of X2", X2_min,"to", X2_max)
            n4 = st.number_input(
                label = "predict range: X2 minimum",key="n4",
                value = round(X2_min,1)
            )
            X2pred_min = n4

            n5 = st.number_input(
                label = "predict range: X2 maximum",key="n5",
                value = round(X2_max,1)
            )
            X2pred_max = n5

            n6 = st.number_input(
                label = "predict range: X2 step", key="n6",
                value = round(X2_step,2)
            )
            X2pred_step = n6

            st.write("")
            st.write("X1 predict: from", X1pred_min,"to",X1pred_max,"step:",X1pred_step)
            selected_1st_axis = np.arange(X1pred_min, X1pred_max+X1pred_step, X1pred_step)
            st.write("X1 will be:",str(selected_1st_axis))
            st.write("")
            st.write("X2 predict: from", X2pred_min,"to",X2pred_max,"step:",X2pred_step)
            selected_2nd_axis = np.arange(X2pred_min, X2pred_max+X2pred_step, X2pred_step)
            ...
            if len(selected_1st_axis)>0 and len(selected_2nd_axis)>0:    
                for element in selected_2nd_axis:
                    df = pd.DataFrame(data=selected_1st_axis, columns=[NNsetting["X1_name"]])
            ...
            if os.path.exists(path_sim):
                ...
                if show_simulation:
                    ...
                    colormap = sns.color_palette("gist_rainbow", N_COLORS).as_hex()
                    fig = px.line(data_frame=df_simulated, x=NNsetting["X1_name"], y=NNsetting["Y_name"], color=NNsetting["X2_name"],
                    color_discrete_sequence=colormap)
                    st.plotly_chart(fig)
            
                if st.checkbox("image plot of simulated result"):
                    df_simulated = pd.read_csv(path_sim)
                    simdata = go.Heatmap(z= df_simulated[NNsetting["Y_name"]].T,
            ...
            if os.path.exists(path_sim):
                with open(path_sim) as f:
                    st.download_button('Download df_simulated.csv', f, file_name='df_simulated.csv')
